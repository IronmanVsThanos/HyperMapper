...
class DPTDinoVisionTransformer(BaseModule):
    def __init__(
        self,
        reins_config=None,
        img_size=224,
    ...
    def forward(self,*args, is_training=False, **kwargs):
        ...
        if self.output_mode == 'seg':
            ...
            patch_h, patch_w = args[0].shape[-2] // 14, args[0].shape[-1] // 14
            features = self.get_intermediate_layers(*args, self.intermediate_layer_idx['vitl'],
            ...
            ret_depth = self.head(ret["x_norm_clstoken"])
            ret = {ret, ret_depth}
        return ret
...
def vit_small(patch_size=16, num_register_tokens=0, **kwargs):
    model = DPTDinoVisionTransformer(
